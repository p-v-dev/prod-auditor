...
```powershell
Since this skill is powered by agnostically structured system instructions (`SKILL.md`), you can use it with **any modern AI coding tool, CLI assistant, or LLM chat**.

### 1. With Terminal Coding Assistants (Claude Code, Kilo Code, Aider)
Simply inject or reference the `SKILL.md` file when starting your session or running a task.

```bash
# Example using Claude Code or similar CLI tools:
claude "Read SKILL.md from this repo and audit my current codebase for production readiness"

# Or if using Aider / Kilo Code:
aider --system-prompt SKILL.md
```
